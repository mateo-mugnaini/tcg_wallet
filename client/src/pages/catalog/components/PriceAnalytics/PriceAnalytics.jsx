...
function PriceAnalytics({ title, variation, aggregations, currency, graded = false }) {
  const maxAverage = Math.max(...(aggregations || []).map((item) => item.averagePrice || 0), 1);
  ...
  return (
    <article className={styles.card}>
      ...
      <div className={styles.chart}>
        <div className={styles.chartHeader}><h3>Promedio por período</h3><span>{aggregations?.length || 0} puntos</span></div>
        {aggregations?.length ? aggregations.map((item) => (
